...
float direccion_float;
int velocidad;   // millas/h; si es en km/h velocidad*1.60934
float velocidad_float;  // Actualmente se usa Km/h
float velocidad_max;  // Para guardar la velocidad máxima y su dirección en el intervalo
int direccion_max;
...
int batteryLow[NUM_ESTACIONES];  //Para informar si la bateria de la Davis esta baja (bit 4 de DATA[0]). 0 ok, 1 Low, -1 No disponible
int RSSI[NUM_ESTACIONES]; //Para informar de la potencia de la señal de la estación (radio.RSSI). 0 No disponible
...
String dataMessage[NUM_ESTACIONES];
...
void setup() {
  ...
  if (cont_mount>=5){
    //Serial.println("Card Mount Failed 5 tried => MSG_error");
    //MSG_error="Card Mount Failed";
  ...
  if (! rtc.initialized() || rtc.lostPower()) {
    ...
    MSG_error=MSG_error + " - RTC is NOT initialized";  
  }
  rtc.start();
  delay(10);
  ...
  for (int i=1; i<NUM_ESTACIONES;i++){ //Ver cuantas estaciones están activas (-1 inactiva)
    ...
    RSSI[i]=0; // No disponible
    batteryLow[i]=-1; //No disponible
  }
...
void loop() {
  ...
  for(int i=0;i<NUM_ESTACIONES;i++){
    ...
    if (stationID<0) dataMessage[i]="No listened";

    if ((stationID>=0)&&(!ServidorON)){
    // Inicicalización:  
    tiempo2 = millis();
    tiempo1 = millis();
    N_humedad=0; humedad=-1; // Reset contadores y medias.
    temperatura=0; N_temperatura=0; temp_float=999.9; 
    N_direccion=0; direccion=-1; 
    velocidad=0; N_velocidad=0; velocidad_float=-1;
    velocidad_max=-1; direccion_max=-1;    
    DataAll=false; // No todos los datos
    RSSI[i]=0; // No disponible
    batteryLow[i]=-1; //No disponible
    ...
    int Nerr=logSDCard(i);   //Guardar en el archivo Datos 
    if (Num_err!=Nerr){
      MSG_error=MSG_error + "- Error: Appending to file";
      Num_err=Nerr;      
    }
...
//--------------------- Radio ------------------
void processPacket(int i) {
  // Every packet has wind speed, direction, and battery status in it

  if ((radio.DATA[1]>0)||(radio.DATA[2]>0)){ //Dato de viento valido
  loopData.windSpeed = radio.DATA[1];
  float aux_vel=loopData.windSpeed*1.60934f; //Si usamos float en la velocidad salen cosas raras, revisar. 
  if (N_velocidad){
    N_velocidad=N_velocidad+1;
    velocidad_float =(aux_vel + (N_velocidad-1)*velocidad_float)/N_velocidad;    
    //velocidad =(loopData.windSpeed + (N_velocidad-1)*velocidad)/N_velocidad; 
    //velocidad= loopData.windSpeed; // NO PROMEDIO
    //velocidad_float=aux_vel;
  }
  else
  {
    N_velocidad=N_velocidad+1;
    velocidad_float = aux_vel;
    velocidad_max = aux_vel;
  ...
  // Uso direccion como entero:
  if (N_direccion){
    N_direccion=N_direccion+1;
    //direccion_float =(loopData.windDirection + (N_direccion-1)*direccion_float)/N_direccion; 
    direccion =(loopData.windDirection + (N_direccion-1)*direccion)/N_direccion; 
  }
  else
  {
    N_direccion=N_direccion+1;
    //direccion_float = loopData.windDirection;
    direccion = loopData.windDirection;
    direccion_max=loopData.windDirection;
  }
  
  if (aux_vel>velocidad_max){
    velocidad_max=aux_vel;    
    direccion_max=loopData.windDirection;
  }
  ...
  batteryLow[i]=int(loopData.transmitterBatteryStatus);

  RSSI[i]=int(radio.RSSI);
  ...
  case VP2P_HUMIDITY:
    ...
    loopData.outsideHumidity = word((radio.DATA[4] >> 4), radio.DATA[3]);
    uint16_t humidity_aux = (int16_t) word((radio.DATA[4] >> 4), radio.DATA[3]);


    if (N_humedad){
      N_humedad=N_humedad+1;
      //humedad =(float(loopData.outsideHumidity / 10.0) + (N_humedad-1)*humedad)/N_humedad; 
      humedad =(float(humidity_aux / 10.0) + (N_humedad-1)*humedad)/N_humedad; 
    }
    else
    {
      N_humedad=N_humedad+1;
      //humedad = float(loopData.outsideHumidity / 10.0);
      humedad = float(humidity_aux / 10.0);
    }
  ...
  if (radio.receiveDone()) {
    ...
    if ((crc == (word(radio.DATA[6], radio.DATA[7]))) && (crc != 0)) {
      ...
      if (correctID){ 
        ...
        processPacket(i);  //JALO: Proceso el paquete si es de la estación correcta y CRC correcto
      }  
      else{
...
int logSDCard(int i) {
  ...
  //Nombre_Fichero = "/Estacion"+String(i+1)+"_"+CD+"_"+CM+"_"+String(anho)+".csv";
  Nombre_Fichero = "/Estacion"+String(i+1)+"_"+String(anho)+"_"+CM+"_"+CD+".csv";   // Cambiar el orden para que se ordene mejor
  
  String bat="--";
  if (batteryLow[i]==0){ bat="Ok";};
  if (batteryLow[i]==1){ bat="Low";};
  String dataMsg = String(dia) + "-" +String(mes) + "-" + String(anho) + ";" + String(hora) + ":" +String(minutos) + ":" + String(segundos) + ";" + String(temp_float) + ";" + String(humedad) + ";" + String(velocidad_float)+ ";"+ String(direccion) +";"+ String(velocidad_max)+ ";"+ String(direccion_max)+ ";" + String(RSSI[i])+";"+bat + "\n";
  dataMessage[i] = "["+String(dia) + "-" +String(mes) + "-" + String(anho) + ";" + String(hora) + ":" +String(minutos) + ":" + String(segundos) + "] - " + String(temp_float) + "ºC - " + String(humedad) + "% - [" + String(velocidad_float)+ "Km/h | "+ String(direccion) + "º]["+ String(velocidad_max)+ "Km/h | "+ String(direccion_max)  + "º]max - RSSI:"+String(RSSI[i])+" - BAT:"+bat;
  //dataMessage = CD + "-" + CM + "-" + String(anho) + ";" + String(hora) + ":" +String(minutos) + ":" + String(segundos) + ";" + String(temp_float) + ";" + String(humedad) + ";" + String(velocidad_float)+ ";"+ String(direccion_float) +"\n";
  Serial.print("Save data: ");
  //Serial.println(dataMsg);
  Serial.println(dataMessage[i]);
  radio.sleep(); //Evitar interrupcion de radio durante la escritura de SD
  int err=appendFile(SD, Nombre_Fichero.c_str(), dataMsg.c_str());
  radio.setChannel(radio.CHANNEL); //Reactivar la radio (se invoca ReceiveBegin())
...
void print_information(int i){
  ...
  Serial.print(F("Estacion ID: "));   Serial.print(stationID); Serial.print(F(" -- Batt: "));
  if (batteryLow[i]==0) { Serial.println(F("Ok"));}; 
  if (batteryLow[i]==1) {Serial.println(F("Low"));} 
  ...
  Serial.print(F(" -- Velocidad: "));   Serial.println(velocidad_float);
...
String processor(const String& var){
  ...
  if(var == "HORA"){
    ...
    return String(dataMessage[1]);
  }else if(var == "DATA3"){
    return String(dataMessage[2]);
  }
  return String();
}
String escucha(){
...
void WiFiEvent(WiFiEvent_t event){
  ...
  server.on("/data1", HTTP_GET, [](AsyncWebServerRequest *request){
    request->send_P(200, "text/plain", data1_actual().c_str());
  });  
  server.on("/data2", HTTP_GET, [](AsyncWebServerRequest *request){
    request->send_P(200, "text/plain", data2_actual().c_str());
  });  
  server.on("/data3", HTTP_GET, [](AsyncWebServerRequest *request){
    request->send_P(200, "text/plain", data3_actual().c_str());
  });  
